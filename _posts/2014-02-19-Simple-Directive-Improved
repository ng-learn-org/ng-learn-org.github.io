...
Even though we add more span/pages in the above HTML, our directive wouldn\'t consider them since the switch block's logic is only covering the first four "span" elements. Let's suppose the following:
...
{% highlight html %}
<div data-my-other-progress-bar 
...
In this case:
  - the directive has one extra attribute -"data-pages"-, which is intended to hold an array of page names. This way we can get rid of the fixed number of pages from our previous simple example.
...
describe("Unit testing other progress bar directive", function() {
  ...
  it("should show 1st span highlighted when Page 1", function() {
    ...
    element = angular.element('<div data-my-other-progress-bar data-currentpage="1" data-currentpagestyle="active" data-pages="['Select Product','Checkout','Confirmation']"></div>');
...
Please note that expectations we're not changed. Even though the code was updated, the result shouldn\'t change.
...
{% highlight javascript %}
  it('should show 1st span highlighted when Page 1', function () {
    element = angular.element('<div data-my-other-progress-bar data-currentpage="1" data-currentpagestyle="selected" data-pages="['Select Product','Confirmation']"></div>');
  ...
  it('should show 6th span highlighted when Page 6', function () {
    element = angular.element('<div data-my-other-progress-bar data-currentpage="6" data-currentpagestyle="selected" data-pages="['Select Product','Product Details','Shipping Info','Billing Info','Confirmation','Share Purchase']"></div>');
  ...
  sampleModule.directive("myOtherProgressBar", [
    function() {
      return {
        ...
        // Example <div my-other-progress-bar> </div>
        restrict: "A",

        templateUrl: "myTemplate.html",

        // isolated scope: directive's inner scope
        // we are mapping the variables from the outer scope into the directive's scope
        scope: {
...
{% highlight html %}
<span ng-repeat="page in allPages" ng-class="{ {{"{{cssActivePage"}}}}: $index + 1 == indexActivePage}">{{"{{page"}}}}</span>
{% endhighlight %}

So now each html has:

{% highlight html %}
<div data-my-other-progress-bar data-currentPage="{{"{{pageIndex"}}}}" data-currentPageStyle="active" data-pages="['Select Product','Confirmation']"></div>
{% endhighlight %}

Let's point out that:
- the directive is declared to be used as an attribute ("restrict" field)
- the directive HTML code related is extracted and placed into a new separated file ("templateUrl" field)
- the directive defines its inner scope with e variables ("scope" field)
- the "link" function does not have any code and because of that it could be prefectly removed (same as "compile" function)
